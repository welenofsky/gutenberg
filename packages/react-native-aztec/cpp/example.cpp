...
void EventHandlerRegistry::processEvent(Runtime &jsiRuntime, int id, const char* eventName, const char* content, int selectionStart, int selectionEnd) {
    ...
    if (handlerIt != eventHandlers.end()) {
        ...
        callback->asFunction(jsiRuntime).call(jsiRuntime, String::createFromUtf8(jsiRuntime, eventName), String::createFromUtf8(jsiRuntime, content), selectionStart, selectionEnd);
    }
}
