...
extern "C"
JNIEXPORT void JNICALL
Java_com_reactnativesimplejsi_SimpleJsiModule_nativeSendEvent(JNIEnv *env, jobject thiz, jlong jsi, jint viewId, jstring eventName, jstring content, jint selectionStart, jint selectionEnd) {
    auto runtime = reinterpret_cast<facebook::jsi::Runtime *>(jsi);
    
    const char* nativeContent = env->GetStringUTFChars(content, 0);
    const char* nativeEventName = env->GetStringUTFChars(eventName, 0);

    example::EventHandlerRegistry::registry->processEvent(*runtime, viewId, nativeEventName, nativeContent, selectionStart, selectionEnd);
